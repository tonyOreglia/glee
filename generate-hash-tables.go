...
func printAllBitboards() {
	...
	defer f.Close()
	f.Write([]byte("\tA FILE"))
	printBitBoard(f, AfileBb)
	f.Write([]byte("\tB FILE:"))
	printBitBoard(f, BfileBb)
	f.Write([]byte("\tC FILE:"))
	printBitBoard(f, CfileBb)
	f.Write([]byte("\tD FILE:"))
	printBitBoard(f, DfileBb)
	f.Write([]byte("\tE FILE:"))
	printBitBoard(f, EfileBb)
	f.Write([]byte("\tF FILE:"))
	printBitBoard(f, FfileBb)
	f.Write([]byte("\tG FILE:"))
	printBitBoard(f, GfileBb)
	f.Write([]byte("\tH FILE:"))
	printBitBoard(f, HfileBb)
	for i := 0; i < 64; i++ {
		f.Write([]byte("\tBITBOARD LOOKUP:"))
		printBitBoard(f, SingleIndexBbHash[i])
		f.Write([]byte("\tNORTH:"))
		printBitBoard(f, NorthArrayBbHash[i])
		f.Write([]byte("\tSOUTH:"))
		printBitBoard(f, SouthArrayBbHash[i])
		f.Write([]byte("\tEAST:"))
		printBitBoard(f, EastArrayBbHash[i])
		f.Write([]byte("\tWEST:"))
		printBitBoard(f, WestArrayBbHash[i])
		f.Write([]byte("\tNORTH EAST:"))
		printBitBoard(f, NorthEastArrayBbHash[i])
		f.Write([]byte("\tNORTH WEST:"))
		printBitBoard(f, NorthWestArrayBbHash[i])
		f.Write([]byte("\tSOUTH EAST:"))
		printBitBoard(f, SouthEastArrayBbHash[i])
		f.Write([]byte("\tSOUTH WEST:"))
		printBitBoard(f, SouthWestArrayBbHash[i])
		f.Write([]byte("\tKNIGHT ATTACK:"))
		printBitBoard(f, KnightAttackBbHash[i])
		f.Write([]byte("\tKING MOVES:"))
		printBitBoard(f, LegalKingMovesBbHash[0][i])
		printBitBoard(f, LegalKingMovesBbHash[1][i])
		f.Write([]byte("\tPAWN MOVES:"))
		printBitBoard(f, LegalPawnMovesBbHash[0][i])
		printBitBoard(f, LegalPawnMovesBbHash[1][i])
		f.Write([]byte("\tEN PASSANT BB LOOKUP BY PAWN DESTINATION: "))
		printBitBoard(f, EnPassantBbHash[i])
		f.Write([]byte("\tATTACKED PAWN LOCATION BY EN PASSANT CAPTURE: "))
		printBitBoard(f, AttackedEnPassantPawnLocationBbHash[i])
	}
}
